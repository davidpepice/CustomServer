...
class Server {
    ...
    mainLoop() {
        ...
        if (this.run) {
            ...
            // Mueva los nodos en movimiento primero
            for (const cell of this.movingNodes) {
                ...
                // Escanee y verifique si hay colisiones de virus / masa expulsada
                this.boostCell(cell);
            ...
            for (const cell of this.nodesPlayer) {
                ...
                // Escanee en busca de colisiones rígidas o de comer y resuélvalas
                const nodes = this.quadTree.allOverlapped(cell.quadItem.bound);
                for (const check of nodes) {
    ...
    boostCell(cell) {
        ...
        var speed = cell.boostDistance / 20; // val: 87
    ...
    resolveCollision(m) {
        ...
        if (cell.owner && cell.owner == check.owner) {
            if (cell.getAge() < 5 || check.getAge() < 5) // 13
                return; // just split => ignore
        }
        else if (check.radius < cell.radius * 1 || !check.canEat(cell)) //0.5
    ...
    spawnPowerUP() {
        var PowerUP = new Entity.PowerUP(this, null, this.randomPos(), 78);
        this.safeSpawn(PowerUP); 
    }
    ...
    spawnPlayer(player, pos) {
        ...
        if (Math.random() <= this.config.ejectSpawnPercent &&
            ...
            size = Math.max(size, eject.radius * .1);
        }
    ...
    ejectMass(client, MacroSpeed) { 
        if (!this.canEjectMass(client) || client.frozen) return;
        var speed = 1;
        MacroSpeed = setInterval(() => { 
        let i = 0;
        for (i ; i < client.cells.length; i++) {
            ...
            // Create cell and add it to node list
            // Crear celda y agregarla a la lista de nodos
            var ejected; 
     ...
     setTimeout(() => {
       clearInterval(MacroSpeed);
     }, 100);
    }  
    canEjectMassMacro(client) {
        if (client.lastEject === null) { // first eject
            client.lastEject = this.ticks;
            return true;
    ...
    ejectMassMacro(client) { 
        if (!this.canEjectMassMacro(client) || client.frozen) return;
